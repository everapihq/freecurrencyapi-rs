pub mod baseline {
    ...
    pub fn construct_client(
    ...
    ) -> Result<Client, FreecurrencyapiError> {
        ...
        let client = Client::builder()
            ...
            .build()
            .map_err(|err| FreecurrencyapiError::ClientConstruction { source: err })?;
        Ok(client)
    }
    ...
    ) -> Result<Url, FreecurrencyapiError> {
        let mut url = Url::parse(BASE_URL).map_err(|_| FreecurrencyapiError::UrlConstruction)?;
